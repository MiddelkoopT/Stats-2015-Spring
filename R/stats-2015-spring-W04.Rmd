...
How do we formulate it?  Simple, for some statistic theta it is 
...
**For mean unknown, and known standard deviation**
...
# Note the difference between Q1 and Q3...
summary(d)
...
hist(d$c,breaks=seq(0,1.6,0.1)-0.05)
hist(d$c,breaks=seq(0,1.6,0.1)-0.05,right=FALSE)

## Back to the confidence interval.
alpha <- 1-0.95
```

**Confidence interval on mu with unknown sigma^2 and large n**
...
## FYI/ the axis is are the same, just different scale (q v.s. %)
pnorm(-1.5) 
pnorm(1.0) 
qnorm(0.05)
```
...
## one could almost guess this button.
boxplot(d$load)
?boxplot
...
* Distribution: $\chio^2$ with $n-1$ degrees of freedom
* Tails: $\chi^2_{\alpha/2,n-1}$ (upper) and $\chi^2_{alpha/2}$ (lower) $\chi^2$ tails of the square distribution
* Two sideded Confindence Interval:
```{r eval=TRUE}
